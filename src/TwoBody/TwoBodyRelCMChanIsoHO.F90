module TwoBodyRelCMChanIsoHO
  use TwoBodyRelCMRadQNumbers
  implicit none
  public :: TwoBodyRelCMChanIsoHOBasis
  private

  type :: TwoBodyRelCMChanIsoHOBasis
  ...
  contains
    procedure :: InitTwoBodyRelCMChanIsoHOBasis
    procedure :: FinTwoBodyRelCMChanIsoHOBasis
    ...
    ! overriding
    generic :: init => InitTwoBodyRelCMChanIsoHOBasis  ! constructor
    generic :: fin => FinTwoBodyRelCMChanIsoHOBasis    ! destructor
    generic :: GetHOs => GetRelCMHO, GetRelCMHOFromQNumbers
    generic :: GetIndex => GetIndexFromQNs, GetIndexFromHO
  end type TwoBodyRelCMChanIsoHOBasis

contains

  function GetJ(this) result(j)
    class(TwoBodyRelCMChanIsoHOBasis), intent(in) :: this
    integer :: j
    j = this%j
  end function GetJ
  ...
  function GetIndexFromHO(this,ho) result(idx)
    ...
    integer :: idx
    idx = this%nls2idx(ho%GetNRel(),ho%GetLRel(),ho%GetSpin(),ho%GetNCM())
  ...
  function GetFrequency(this) result(hw)
    class(TwoBodyRelCMChanIsoHOBasis), target, intent(in) :: this
    real(8) :: hw
    hw = this%hw
  ...
  subroutine FinTwoBodyRelCMChanIsoHOBasis(this)
    class(TwoBodyRelCMChanIsoHOBasis), intent(inout) :: this
    deallocate(this%nls2idx)
    deallocate(this%HOn)
  end subroutine FinTwoBodyRelCMChanIsoHOBasis

  subroutine InitTwoBodyRelCMChanIsoHOBasis(this, hw, Nmax, Nmax_rel, j, p, z, j_rel, l_cm, Nmax_cm)
    class(TwoBodyRelCMChanIsoHOBasis), intent(inout) :: this
    real(8), intent(in) :: hw
  ...
  end subroutine InitTwoBodyRelCMChanIsoHOBasis

end module TwoBodyRelCMChanIsoHO
