module TwoBodyRelCMSpaceIsoHO
  use TwoBodyRelCMChanIsoHO
  implicit none
  public :: TwoBodyRelCMSpaceIsoHOBasis
  private

  type :: TwoBodyRelCMSpaceIsoHOBasis
    type(TwoBodyRelCMChanIsoHOBasis), allocatable :: jpz_jrel_lcm(:)
    real(8), private :: hw
    integer, private :: Nmax, Nmax_rel, NcmMax, Jmax, NChan, j_rel_max, LcmMax
    integer, private, allocatable :: jpz_jrel_lcm2idx(:,:,:,:,:)
    logical, private :: initialized = .false.
  contains
    procedure :: InitTwoBodyRelCMSpaceIsoHOBasis
    procedure :: FinTwoBodyRelCMSpaceIsoHOBasis
    ...
    ! overriding
    generic :: init => InitTwoBodyRelCMSpaceIsoHOBasis
    generic :: fin => FinTwoBodyRelCMSpaceIsoHOBasis
    generic :: GetChannel => GetChannelFromIndex, GetChannelFromQNumbers
  end type TwoBodyRelCMSpaceIsoHOBasis

contains

  function GetFrequency(this) result(hw)
    class(TwoBodyRelCMSpaceIsoHOBasis), intent(in) :: this
    real(8) :: hw
    hw = this%hw
  ...
  function GetChannelFromIndex(this, idx) result(r)
    class(TwoBodyRelCMSpaceIsoHOBasis), target, intent(in) :: this
    integer, intent(in) :: idx
    type(TwoBodyRelCMChanIsoHOBasis), pointer :: r
    r => null()
    if(idx < 1) return
  ...
  subroutine FinTwoBodyRelCMSpaceIsoHOBasis(this)
    class(TwoBodyRelCMSpaceIsoHOBasis), intent(inout) :: this
    integer :: ich
    do ich = 1, this%NChan
  ...
  end subroutine FinTwoBodyRelCMSpaceIsoHOBasis

  subroutine InitTwoBodyRelCMSpaceIsoHOBasis(this, hw, Nmax, Jmax, Nmax_rel, j_rel_max, NcmMax, LcmMax)
    class(TwoBodyRelCMSpaceIsoHOBasis), intent(inout) :: this
  ...
  end subroutine InitTwoBodyRelCMSpaceIsoHOBasis
end module TwoBodyRelCMSpaceIsoHO

!program test
!  use RelativeSpaceHarmonicOscillator
!  implicit none
!  type(TwoBodyRelSpaceIsoHOBasis) :: ho
